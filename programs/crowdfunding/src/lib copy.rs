...
pub mod crowdfunding {
    ...
    //with draw token from pda of admin
    pub fn withdraw_token_from_pda(
        ctx: Context<WithdrawTokenFromPda>,
        amount: u64,
        _ido_id: u32
    ) -> Result<()> {
        ...
        //check admin
        require!( ido_account._is_admin(&ctx.accounts.payer.key),
            IDOProgramErrors::NotAuthorized
        );
    ...
    pub fn claim(ctx: Context<ClaimToken>, index: u16, claimant: Pubkey) -> Result<()> {
        let ido_account = &mut ctx.accounts.ido_account;
        //check release token
...
trait IdoStrait {
    ...
    //getter function
    fn get_info_ido(&self) -> IdoAccount;
